...
unsigned long crc_mem(unsigned char *p, unsigned long len, unsigned long crc) {
	do {
//    printf("%c(%d):\t%08X\n", *p, *p, crc);
...
// Считаем crc
unsigned long crc_file(char* pszFileName, unsigned long crc) {
  // А может не надо считать?
  String strCRC=GetFileVersion(pszFileName, "SourceCRC");
  if(!strCRC.IsEmpty()) {
    unsigned char buf[]="\0\0\0\0";
    HexToBin(strCRC.LowerCase().c_str(), buf, 4);
    unsigned char c;
    c = buf[0];
    buf[0] = buf[3];
    buf[3] = c;
  ...
  // Нет, надо
  TFilePtr inp(pszFileName, "rb");
  fseek(inp, 0, 2);
  unsigned long len = ftell(inp);
  boost::scoped_array<unsigned char> data(new unsigned char[len]);
  fseek(inp, 0, 0);
  fread(data.get(), len, 1, inp);
  return crc_mem(data.get(), len, crc);
}
//---------------------------------------------------------------------------

// Тянем версию файла
String GetFileVersion(const String &strFileName, const String &strDataType) {
  ...
  boost::scoped_array<char> t(pszVersion);
  return pszVersion;
}
//---------------------------------------------------------------------------
